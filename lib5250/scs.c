...
void scs_sea(Tn5250SCS* This) {
    ...
    for (loop = 0; loop < This->curchar - 2; loop++) {
        ...
        if (action > 3) {
            if (This->usesyslog) {
                SCS_LOG("Invalid action (exception class: %d, action %d)",
                        exception, action);
            }
            fprintf(stderr, "Invalid action (exception class: %d, action %d)\n",
                    exception, action);
        }
        else {
#ifdef DEBUG
            ...
            case 0: {
                if (exception == 0) {
                    if (This->usesyslog) {
                        SCS_LOG("Using action ACCEPT for exception class %d "
                                "(all exception classes)",
                                exception);
                    }
                    fprintf(stderr,
                            "\tUsing action ACCEPT for exception class %d (all "
                            "exception classes)",
                            exception);
                }
                else {
            ...
            case 1: {
                if (exception == 0) {
                    if (This->usesyslog) {
                        SCS_LOG("Using action IGNORE for exception class %d "
                                "(all exception classes)",
                                exception);
                    }
                    fprintf(stderr,
                            "\tUsing action IGNORE for exception class %d (all "
                            "exception classes)",
                            exception);
                }
                else {
            ...
            case 2: {
                if (exception == 0) {
                    if (This->usesyslog) {
                        SCS_LOG("Using action TERMINATE for exception class %d "
                                "(all exception classes)",
                                exception);
                    }
                    fprintf(stderr,
            ...
            case 3: {
                if (exception == 0) {
                    if (This->usesyslog) {
                        SCS_LOG("Using action SUSPEND for exception class %d "
                                "(all exception classes)",
                                exception);
                    }
                    fprintf(stderr,
...
void scs_ppm(Tn5250SCS* This) {
    ...
    if (This->curchar > 6) {
        ...
        if (This->usesyslog) {
            SCS_LOG("Destination drawer offset = %x", destdraweroffset);
        }
#ifdef DEBUG
...
#ifdef VERBOSE
    fprintf(stderr, "\tPrintable region is %d by %d inches", (width / 1440),
            (length / 1440));
#endif
    fprintf(stderr, "\n");
#endif
...
void scs_sto(Tn5250SCS* This) {
    ...
    case 0xFF: {
        ...
        if (pagerot2 == 0xFE) {
            ...
            if (This->usesyslog) {
                SCS_LOG("STO used unhandled page rotation: Setting text "
                        "orientation based on SPPS command");
            }
#ifdef VERBOSE
            fprintf(stderr,
...
void scs_sfg(Tn5250SCS* This) {
    ...
    if (This->usesyslog) {
        SCS_LOG("SFG set global font ID %x%x", globalfontid1, globalfontid2);
    }
...
void scs_ahpp(Tn5250SCS* This) {
    ...
    if ((This->usesyslog) && (This->loglevel > 0)) {
        SCS_LOG("PP sent absolute horizontal move of %d (cursor currently on "
                "column %d)",
...
void scs_irt(Tn5250SCS* This) {
    if ((This->usesyslog) && (This->loglevel > 0)) {
        SCS_LOG("Doing Index Return (new line and reset indent level)");
    }
#ifdef DEBUG
    fprintf(stderr, "IRT\n");
#endif
 ...
 */
void scs_setfont(Tn5250SCS* This) { return; }
