...
static void tn5250_session_define_selection_item(Tn5250Session* This,
                                                 ...
                                                 short createnew);
static void tn5250_session_create_window_structured_field(Tn5250Session* This,
                                                          int length);
static void tn5250_session_define_scrollbar(Tn5250Session* This, int length);
static void
tn5250_session_remove_gui_window_structured_field(Tn5250Session* This,
                                                  int length);
static void
tn5250_session_remove_all_gui_constructs_structured_field(Tn5250Session* This,
                                                          int length);
static void tn5250_session_write_data_structured_field(Tn5250Session* This,
                                                       int length);
...
static void tn5250_session_handle_receive(Tn5250Session* This) {
    ...
    while (tn5250_stream_record_count(This->stream) > 0) {
        ...
        TN5250_LOG(
            ("HandleReceive: cur_opcode = 0x%02X %d\n", cur_opcode, atn));
...
static void tn5250_session_send_fields(Tn5250Session* This, int aidcode) {
    ...
    TN5250_LOG(
        ("SendFields: row = %d; col = %d; aid = 0x%02x\n", Y, X, aidcode));
    ...
    default: /* Huh? */
        TN5250_LOG(("BUG!!! Trying to transmit fields when This->read_opcode = "
                    "0x%02X.\n",
                    This->read_opcode));
        TN5250_ASSERT(0);
    }
    ...
    if (field->continuous) {
        ...
        if (tn5250_field_is_continued_first(field)) {
            ...
            for (iter = field; iter->continuous; iter = iter->next) {
                memcpy(data + i, tn5250_display_field_data(This->display, iter),
    ...
    case CMD_READ_IMMEDIATE:
        if (tn5250_field_is_signed_num(field)) {
            ...
            tn5250_buffer_append_byte(
                buf,
                tn5250_char_map_to_local(tn5250_display_char_map(This->display),
                                         data[size - 1]) == '-'
                    ? (0xd0 | (0x0f & c))
                    : c);
        }
...
static void tn5250_session_write_error_code(Tn5250Session* This, int readop) {
    ...
    while (1) {
        ...
        if (tn5250_char_map_printable_p(tn5250_display_char_map(This->display),
                                        c)) {
            tempmsg[msglen] = c;
            msglen++;
            continue;
...
static void tn5250_session_write_to_display(Tn5250Session* This) {
    ...
    if (end_y != 0xff && end_x != 0xff &&
        !(CC2 & TN5250_SESSION_CTL_IC_ULOCK)) {
        tn5250_display_set_cursor(This->display, end_y, end_x);
    }
...
static void tn5250_session_start_of_field(Tn5250Session* This) {
    ...
    if ((cur_char & 0xe0) != 0x20) {
        ...
        TN5250_LOG(
            ("StartOfField: field format word = 0x%02X%02X\n", FFW1, FFW2));
        cur_char = tn5250_record_get_byte(This->record);
    ...
    if (input_field) {
        ...
        TN5250_LOG(
            ("StartOfField: endrow = %d; endcol = %d\n", endrow, endcol));
...
static void tn5250_session_write_extended_attribute(Tn5250Session* This) {
    ...
    TN5250_LOG(
        ("WEA order (type = 0x%02X, attribute = 0x%02X).\n", attrtype, attr));
...
static void tn5250_session_write_display_structured_field(Tn5250Session* This) {
    ...
    default:
        TN5250_LOG(
            ("tn5250_write_display_structured_field: Invalid SF Class: %02x\n",
             type));
        errorcode = TN5250_NR_INVALID_SF_CLASS_TYPE;
        tn5250_session_send_error(This, errorcode);
        return;
...
static void tn5250_session_repeat_to_address(Tn5250Session* This) {
    ...
    if (temp[0] == 0 || temp[0] > height || temp[1] == 0 || temp[1] > width ||
        end < start) {
        errorcode = TN5250_NR_INVALID_ROW_COL_ADDR;
        tn5250_session_send_error(This, errorcode);
        return;
...
static void tn5250_session_write_structured_field(Tn5250Session* This) {
    ...
    if (temp[2] != 0xD9) {
        ...
        default:
            TN5250_LOG(
                ("tn5250_write_structured_field(2): Invalid SF Class: %02x\n",
                 temp[3]));
            errorcode = TN5250_NR_INVALID_SF_CLASS_TYPE;
            tn5250_session_send_error(This, errorcode);
            return;
...
static void tn5250_session_read_cmd(Tn5250Session* This, int readop) {
    ...
    TN5250_LOG(
        ("tn5250_session_read_cmd: CC1 = 0x%02X; CC2 = 0x%02X\n", CC1, CC2));
    tn5250_display_indicator_clear(This->display,
                                   TN5250_DISPLAY_IND_X_SYSTEM |
...
static void tn5250_session_define_selection_field(Tn5250Session* This,
    ...
    if ((menubar = tn5250_menubar_hit_test(
             dbuffer->menubar_list, tn5250_display_cursor_x(This->display),
             tn5250_display_cursor_y(This->display))) == NULL) {
    ...
    if ((flagbyte1 & 0xC0) == 3) {
        ...
        if (flagbyte1 & 0x40) {
            TN5250_LOG(("Use this selection field only if the display does not "
                        "have a mouse\n"));
        }
        if (flagbyte1 & 0x80) {
            TN5250_LOG(
                ("Use this selection field only if the display has a mouse\n"));
        }
    }
    ...
    if ((flagbyte1 & 0x0C) == 0) {
        ...
        TN5250_LOG(("Selection field is auto-enter on selection except if "
                    "double-digit numeric selection is used\n"));
    }
    else if ((flagbyte1 & 0x0C) == 2) {
        TN5250_LOG(("Selection field is auto-enter on selection or deselection "
                    "except if double-digit numeric selection is used\n"));
    }
    else {
        TN5250_LOG(
            ("Selection field is auto-enter on selection except if "
             "single-digit or double-digit numeric selection is used\n"));
    }
    ...
    if (flagbyte3 & 0x80) {
        TN5250_LOG(
            ("Make selected choices available when keyboard is unlocked\n"));
    }
    ...
    TN5250_LOG(
        ("textsize = 0x%02X (%d)\n", menubar->itemsize, menubar->itemsize));
    ...
    if (createnewmenubar) {
        ...
        tn5250_dbuffer_add_menubar(tn5250_display_dbuffer(This->display),
                                   menubar);
        tn5250_terminal_create_menubar(This->display->terminal, This->display,
                                       menubar);
    }
    ...
    while (length > 0) {
        ...
        if (reserved == 0x10) {
            tn5250_session_define_selection_item(
                This, menubar, minorlength, menuitemcount, createnewmenubar);
            menuitemcount++;
...
static void tn5250_session_define_selection_item(Tn5250Session* This,
    ...
    if (createnew) {
        ...
        menuitem =
            tn5250_menuitem_list_find_by_id(menubar->menuitem_list, count);
    }
    ...
    if ((flagbyte1 & 0x03) == 0) {
        TN5250_LOG(("Numeric selection characters are not included in this "
                    "minor structure\n"));
    }
    else if ((flagbyte1 & 0x0C) == 1) {
        TN5250_LOG(("A single-digit numeric selection character is included in "
                    "this minor structure\n"));
        selectchars_incl = 1;
    }
    else if ((flagbyte1 & 0x0C) == 2) {
        TN5250_LOG(("Double-digit numeric selection characters are included in "
                    "this minor structure\n"));
        selectchars_incl = 1;
    }
    else {
    ...
    if (flagbyte2 & 0x40) {
        TN5250_LOG(("application user desires a roll-down AID if the Cursor Up "
                    "key is pressed on this choice\n"));
    }

    if (flagbyte2 & 0x20) {
        TN5250_LOG(("application user desires a roll-up AID if the Cursor Down "
                    "key is pressed on this choice\n"));
    }

    if (flagbyte2 & 0x10) {
        TN5250_LOG(("application user desires a roll-left AID if the Cursor "
                    "Left key is pressed on this choice\n"));
    }

    if (flagbyte2 & 0x08) {
        TN5250_LOG(("application user desires a roll-right AID if the Cursor "
                    "Right key is pressed on this choice\n"));
    }
    ...
    if (flagbyte3 & 0x80) {
        TN5250_LOG(("use this minor structure for GUI devices (including "
                    "GUI-like NWSs)\n"));
    }

    if (flagbyte3 & 0x40) {
        TN5250_LOG(("use this minor structure for non-GUI NWSs that are "
                    "capable of creating mnemonic underscores\n"));
    }

    if (flagbyte3 & 0x20) {
        TN5250_LOG(("use this minor structure for NWS display devices that are "
                    "not capable of creating underscores\n"));
    }
    ...
    if (createnew) {
        tn5250_terminal_create_menuitem(This->display->terminal, This->display,
                                        menuitem);
    }
...
static void tn5250_session_remove_gui_selection_field(Tn5250Session* This,
    ...
    if (tn5250_dbuffer_menubar_count(This->display->display_buffers) > 0) {
        tn5250_terminal_destroy_menubar(
            This->display->terminal, This->display,
            This->display->display_buffers->menubar_list);
        This->display->display_buffers->menubar_list =
            tn5250_menubar_list_destroy(
                This->display->display_buffers->menubar_list);
...
static void tn5250_session_create_window_structured_field(Tn5250Session* This,
    ...
    if (flagbyte1 & 0x40) {
        /*
           TN5250_LOG (("Pull down window - data ignored (will use selection
           field data)\n"));
         */
        ...
        if (border_type == 0x01) {
            if (border_length > 0) {
                ...
                if (border_flags & 0x80) {
                    TN5250_LOG((
                        "Use border presentation characters (GUI-like NWS)\n"));
                }
                border_length--;
                length--;
            }
            if (border_length > 0) {
                ...
                TN5250_LOG(
                    ("Monochrome border attribute = 0x%02X\n", border_mono));
                border_length--;
                length--;
            }
            if (border_length > 0) {
            ...
            if (border_length > 0) {
                ...
                TN5250_LOG(
                    ("Bottom border character = 0x%02X\n", border_bottomchar));
                border_length--;
                length--;
            }
            if (border_length > 0) {
    ...
    tn5250_display_erase_region(
        This->display, window->row + 1, window->column + 2,
        window->row + window->height + 1, window->column + window->column + 2,
        window->column + 2, window->column + window->width + 2);
    /*
       }
     */
    return;
...
static void tn5250_session_define_scrollbar(Tn5250Session* This, int length) {
    ...
    if ((scrollbar = tn5250_scrollbar_hit_test(
             dbuffer->scrollbar_list,
             tn5250_display_cursor_x(This->display) + 1,
             tn5250_display_cursor_y(This->display) + 1)) == NULL) {
    ...
    length--;
    scrollbar->rowscols = (1000 * (int)totalrowscols1) +
                          (100 * (int)totalrowscols2) +
                          (10 * (int)totalrowscols3) + (int)totalrowscols4;
    TN5250_LOG(
        ("Total rows/columns that can be scrolled: %d\n", scrollbar->rowscols));
    ...
    length--;
    scrollbar->sliderpos = (1000 * (int)sliderpos1) + (100 * (int)sliderpos2) +
...
tn5250_session_remove_gui_window_structured_field(Tn5250Session* This,
    ...
    TN5250_LOG(
        ("Entering tn5250_session_remove_gui_window_structured_field()\n"));
