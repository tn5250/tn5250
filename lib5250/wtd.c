 ...
 *****/
Tn5250WTDContext* tn5250_wtd_context_new(Tn5250Buffer* buffer,
                                         Tn5250DBuffer* sd, Tn5250DBuffer* dd) {
...
static void tn5250_wtd_context_convert_nosrc(Tn5250WTDContext* This) {
    ...
    for (This->y = 0; This->y < tn5250_dbuffer_height(This->dst); This->y++) {
        for (This->x = 0; This->x < tn5250_dbuffer_width(This->dst);
             This->x++) {
            if ((window = tn5250_window_hit_test(This->dst->window_list,
                                                 This->x + 1, This->y + 1)) !=
                NULL) {
                tn5250_wtd_context_write_cwsf(This, window);
            }
            else if ((menubar = tn5250_menubar_hit_test(
                          This->dst->menubar_list, This->x, This->y)) != NULL) {
...
static void tn5250_wtd_context_write_cwsf(Tn5250WTDContext* This,
    ...
    TN5250_LOG(("window:\n\tid: %d\n\trow: %d\n\tcolumn: %d\n\theight: "
                "%d\n\twidth: %d\n",
                window->id, window->row, window->column, window->height,
                window->width));
...
static void tn5250_wtd_context_write_dsfsf(Tn5250WTDContext* This,
    ...
    do {
        minorlength = minorlength + 6 + strlen((char*)iter->text);
        iter = iter->next;
    ...
    iter = menubar->menuitem_list;
    do {
        minorlength = 6 + strlen((char*)iter->text);
        ...
        tn5250_wtd_context_putc(This, 0x00);
        for (i = 0; i < strlen((char*)iter->text); i++) {
...
static void tn5250_wtd_context_write_dsbfsf(Tn5250WTDContext* This,
    ...
    TN5250_LOG(("scrollbar:\n\tid: %d\n\tdirection: %d\n\trowscols: "
                "%d\n\tsliderpos: %d\n\tsize: %d\n",
                scrollbar->id, scrollbar->direction, scrollbar->rowscols,
                scrollbar->sliderpos, scrollbar->size));
