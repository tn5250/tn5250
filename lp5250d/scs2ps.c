...
Tn5250SCS* tn5250_scs2ps_new() {
    ...
    if (scs == NULL) {
        fprintf(stderr, "Unable to allocate memory in tn5250_scs2ps_new ()!\n");
        return NULL;
    }
...
float scs2ps_getx() { return lm + (ccp - 1) * charwidth; }

float scs2ps_gety() { return pl - (tm + (current_line * charheight)); }
