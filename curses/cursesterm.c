...
static void
curses_terminal_update_indicators(Tn5250Terminal* This,
                                  Tn5250Display* display) /*@modifies This@*/;
...
#else
#define TN5250_MOUSE_MASK (BUTTON1_CLICKED)
#endif
...
static void curses_terminal_update(Tn5250Terminal* This,
    ...
    if (This->data->last_width != tn5250_display_width(display) ||
        ...
        for (x = 0; x < 10; x++) {
            refresh();
            if (tn5250_display_width(display) ==
                curses_terminal_width(This) - 1) {
                break;
            }
            usleep(10000);
        }
 ...
 *    DOCUMENT ME!!!
 *****/
static void curses_terminal_update_indicators(Tn5250Terminal /*@unused@*/* This,
                                              Tn5250Display* display) {
...
static int curses_terminal_getkey(Tn5250Terminal* This) {
    ...
    while (1) {
        ...
        case 0x1b:
            if ((key = curses_terminal_get_esc_key(This, 1)) != ERR) return key;
            break;
        ...
        case K_CTRL('G'): /* C-g <function-key-shortcut> */
            if ((key = curses_terminal_get_esc_key(This, 0)) != ERR) return key;
            break;
 ...
 *****/
void curses_terminal_print_screen(Tn5250Terminal* This,
                                  Tn5250Display* display) {
    ...
    if (This->data->config != NULL) {
        ...
        if (tn5250_config_get(This->data->config, "psfontsize_80")) {
            fs132 =
                atoi(tn5250_config_get(This->data->config, "psfontsize_132"));
        }
    ...
    prttext = malloc(
        (2 * tn5250_display_width(display) * tn5250_display_height(display)) +
        1);
    ...
    fprintf(out, "/blkbox {       %% draw a black box behind the text\n"
                 ...
                 "} bind def\n");
    fprintf(out,
            "/borderbox { %% Print a border around screen dump\n"
            "   gsave\n"
            "   newpath\n"
            "   0 setlinewidth\n"
            "   0 setgray\n"
            ...
            "   grestore\n"
            "} bind def\n",
            tn5250_display_width(display), tn5250_display_height(display) + 1,
            tn5250_display_width(display));
