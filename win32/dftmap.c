...
HRESULT CreateLink(LPCSTR lpszPathObj, LPCSTR lpszArgs, LPSTR lpszPathLink,
                   LPSTR lpszDesc);
...
char* maplist[] = { "United States (CCSID 37)",
                    "Canada (CCSID 37)",
                    "Portugal (CCSID 37)",
                    "Brazil (CCSID 37)",
                    "Australia (CCSID 37)",
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, PSTR cmdLine,
                   int iCmdShow) {
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            if (x == LB_ERR) {
                MessageBox(NULL, "You must select a character map!", "Setup",
                           MB_OK);
            }
            else {
                char* buf;
...
void use_options_file(const char* fn, FILE* rcfile, const char* path) {
    ...
    while (fgets(buf, 1024, optf) != NULL) {
        ...
        if ((*buf) == '\0' || (*buf) == '#') {
            ...
            char msg[2048];
            sprintf(msg,
                    "This installation has been customized for\n"
                    "\t%s\n"
...
void install_icon(const char* buf, const char* path) {
    ...
    *name = '\0';
    ...
    if (strcmp(type, "printer") == 0 || strcmp(type, "print") == 0 ||
        strcmp(type, "lp5250") == 0 || strcmp(type, "lp5250d") == 0) {
        strcpy(pgmname, "LP5250D.EXE");
    }
    else if (strcmp(type, "display") == 0 || strcmp(type, "terminal") == 0 ||
...
HRESULT CreateLink(LPCSTR lpszPathObj, LPCSTR lpszArgs, LPSTR lpszPathLink,
    ...
    if (SUCCEEDED(hres)) {
        ...
        if (hres == REGDB_E_CLASSNOTREG) {
            MessageBox(NULL, "CoCreateInstance: Class Not Registered", "Setup",
                       MB_OK);
        }
        else if (hres == CLASS_E_NOAGGREGATION) {
            MessageBox(NULL,
                       "CoCreateInstance: Class cannot be part of an"
                       " Aggregate",
                       "Setup", MB_OK);
        }
        else {
...
HRESULT GetShellFolderPath(int nFolder, LPSTR lpszShellPath) {
    ...
    if (!SUCCEEDED(hres)) {
        sprintf(msg,
                "GetShellFolderPath: SHGetSpecialFolderLocation"
                " failed. (folder=0x%X, hresult 0x%X)",
                nFolder, hres);
        MessageBox(NULL, msg, "Setup", MB_OK);
    ...
    if (SHGetPathFromIDList(pidl, szDir) == FALSE) {
        ...
        pShellMalloc->lpVtbl->Release(pShellMalloc);
        MessageBox(NULL,
                   "GetShellFolderPath: SHGetPathFromIDList"
                   " failed",
                   "Setup", MB_OK);
        return S_FALSE;
    }
