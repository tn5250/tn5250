...
void tn5250_win32_init_fonts(Tn5250Terminal* This, const char* myfont80,
                             const char* myfont132);
static void win32_terminal_font(Tn5250Terminal* This, const char* fontname,
                                int cols, int rows, int fontwidth,
                                int fontheight);
void win32_parse_fontspec(const char* fontspec, char* fontname, int maxlen,
                          int* w, int* h);
void win32_calc_default_font_size(HWND hwnd, int cols, int rows, int* w,
                                  int* h);
...
void win32_terminal_draw_text(HDC hdc, int attr, const char* text, int len,
                              int x, int y, int* spacing,
                              Tn5250Win32Attribute* map, int ox, int oy);
LRESULT CALLBACK win32_terminal_wndproc(HWND hwnd, UINT msg, WPARAM wParam,
                                        LPARAM lParam);
...
static void win32_do_terminal_update(HDC hdc, Tn5250Terminal* This,
                                     Tn5250Display* display,
                                     Tn5250Win32Attribute* map, int ox, int oy);
void win32_move_caret_to(Tn5250Terminal* This, Tn5250Display* disp, short y,
                         short x);
 ...
 *****/
static int tn5250_win32_terminal_enhanced(Tn5250Terminal* This) { return 0; }
...
static void win32_terminal_init(Tn5250Terminal* This) {
    ...
    if (This->data->config != NULL) {
        ...
        if (tn5250_config_get(This->data->config, "ruler")) {
            tn5250_win32_terminal_display_ruler(
                This, tn5250_config_get_bool(This->data->config, "ruler"));
        }
        ...
        if (tn5250_config_get(This->data->config, "pcspeaker")) {
            ...
            tn5250_win32_set_beep(
                This, tn5250_config_get(This->data->config, "beepfile"));
        }
        ...
        if (tn5250_config_get(This->data->config, "click_moves_cursor")) {
            This->data->click_moves_caret = tn5250_config_get_bool(
                This->data->config, "click_moves_cursor");
        }
        ...
        while (colorlist[i].name != NULL) {
            if (tn5250_parse_color(This->data->config, colorlist[i].name, &r,
                                   &g, &b) != -1) {
    ...
    wndclass.hIcon =
        LoadIcon(This->data->hInst, MAKEINTRESOURCE(IDI_TN5250_ICON));
    wndclass.hIconSm =
        LoadIcon(This->data->hInst, MAKEINTRESOURCE(IDI_TN5250_ICON));
    ...
    This->data->hwndMain = CreateWindow(
        TN5250_WNDCLASS, title,
        This->data->resize_fonts ? WS_OVERLAPPEDWINDOW
                                 : (WS_OVERLAPPEDWINDOW & (~WS_MAXIMIZEBOX)),
        CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL,
        This->data->hInst, NULL);
    free(title);
...
void tn5250_win32_init_fonts(Tn5250Terminal* This, const char* myfont80,
    ...
    win32_calc_default_font_size(This->data->hwndMain, 80, RowsFor80Col + 1,
                                 &default_w, &default_h);
...
void win32_parse_fontspec(const char* fontspec, char* fontname, int maxlen,
    ...
    *w = atoi(width);
    ...
    TN5250_LOG(
        ("WIN32: font name=%s, height=%d, width=%d\n", fontname, *h, *w));
    return;
}
...
void win32_calc_default_font_size(HWND hwnd, int cols, int rows, int* w,
    ...
    *w = cli_width / cols;
    ...
    TN5250_LOG(
        ("WIN32: defaulting font size to height=%d, width=%d\n", *h, *w));
...
static void win32_terminal_font(Tn5250Terminal* This, const char* fontname,
    ...
    TN5250_LOG(("WIN32: Using font size:  Loaded with height=%d, width=%d, got "
                "height=%d, width=%d\n",
                fontheight, fontwidth, This->data->font_height,
    ...
    if (globDisplay != NULL) {
        This->data->caretx =
            tn5250_display_cursor_x(globDisplay) * This->data->font_width;
        This->data->carety =
            tn5250_display_cursor_y(globDisplay) * This->data->font_height;
    }
    ...
    if ((!This->data->maximized) && (!This->data->dont_auto_size)) {
        TN5250_LOG(
            ("WIN32: Re-sizing window to %d by %d\n", win_width, win_height));
...
static void win32_do_terminal_update(HDC hdc, Tn5250Terminal* This,
                                     Tn5250Display* display,
                                     Tn5250Win32Attribute* map, int ox,
                                     int oy) {
    int y, x;
    int mx, my;
...
void win32_terminal_draw_text(HDC hdc, int attr, const char* text, int len,
    ...
    if (flags & A_UNDERLINE) {
        HPEN savepen;
        savepen =
            SelectObject(hdc, CreatePen(PS_SOLID, 0, map[attr - 0x20].fg));
    ...
    if (flags & A_VERTICAL && colsep_style == COLSEPSTYLE_FULL) {
        HPEN savepen;
        if (flags & A_REVERSE) {
            savepen = SelectObject(
                hdc, CreatePen(PS_SOLID, 0, colorlist[A_5250_BLACK].ref));
        }
        else {
            savepen =
                SelectObject(hdc, CreatePen(PS_SOLID, 0, map[attr - 0x20].fg));
...
static void win32_terminal_update_indicators(Tn5250Terminal* This,
    ...
    win32_terminal_draw_text(bmphdc, 0x22, ind_buf, 79, 0,
                             ...
                             This->data->spacing, attribute_map, 0, 0);

    This->data->caretx =
        tn5250_display_cursor_x(display) * This->data->font_width;
    This->data->carety =
        tn5250_display_cursor_y(display) * This->data->font_height;
    ...
    if (This->data->display_ruler) {
        ...
        GetClientRect(This->data->hwndMain, &rect);
        savepen = SelectObject(
            bmphdc, CreatePen(PS_SOLID, 0, colorlist[A_5250_RULER_COLOR].ref));
 ...
 * DESCRIPTION
 *    Pops a dialog box and let the user choose a new font for the current
 *display.
 *****/
void win32_terminal_choosefont(HWND hwnd) {
    ...
    if (ChooseFont(&cf)) {
        ...
        if (globTerm->data->font_in_use.lfHeight < 0) {
            globTerm->data->font_in_use.lfHeight =
                -globTerm->data->font_in_use.lfHeight;
        }

        sprintf(fontName, "%s-%dx%d", globTerm->data->font_in_use.lfFaceName,
                globTerm->data->font_in_use.lfWidth,
                globTerm->data->font_in_use.lfHeight);
...
LRESULT CALLBACK win32_terminal_wndproc(HWND hwnd, UINT msg, WPARAM wParam,
    ...
    case WM_COMMAND:
        ...
        case IDM_EDIT_SELECT_ALL:
            ...
            globTerm->data->selend.x = (tn5250_display_width(globDisplay) *
                                        globTerm->data->font_width) -
                                       2;
            globTerm->data->selend.y = (tn5250_display_height(globDisplay) *
                                        globTerm->data->font_height) -
                                       2;
    ...
    case WM_SIZE:
        ...
        if (h > 0 && w > 0) {
            ...
            if (globTerm != NULL && globDisplay != NULL) {
                if (globTerm->data->resize_fonts) {
                    win32_calc_default_font_size(hwnd, 80, RowsFor80Col + 1, &c,
                                                 &r);
                    globTerm->data->font_80_h = r;
                    globTerm->data->font_80_w = c;
                    win32_calc_default_font_size(hwnd, 132, RowsFor132Col + 1,
                                                 &c, &r);
    ...
    case WM_KEYUP:
        ...
        while (keyup2msg[x].win32_key != -1) {
            if ((int)wParam == keyup2msg[x].win32_key &&
                (keyup2msg[x].ctx || !handledkey) && keyup2msg[x].ext == ext &&
                (ks[keyup2msg[x].keystate] & 0x80)) {
                win32_terminal_queuekey(hwnd, globTerm, keyup2msg[x].func_key);
                PostMessage(hwnd, WM_TN5250_KEY_DATA, 0, 0);
    ...
    case WM_KEYDOWN:
        ...
        while (keydown2msg[x].win32_key != -1) {
            if ((int)wParam == keydown2msg[x].win32_key &&
                keydown2msg[x].ctx == ctx && keydown2msg[x].ext == ext &&
        ...
        if (lReturn == 0) {
            ...
            if (globTerm->data->selected) {
                ...
                Rectangle(hdc, globTerm->data->selstr.x,
                          globTerm->data->selstr.y, globTerm->data->selend.x,
    ...
    case WM_LBUTTONUP:
        ...
        if (globTerm->data->click_moves_caret && globDisplay != NULL) {
            win32_move_caret_to(globTerm, globDisplay, (short)HIWORD(lParam),
                                (short)LOWORD(lParam));
            redraw = 1;
        }
        if (globTerm->data->selecting && globDisplay != NULL) {
...
void win32_make_new_caret(Tn5250Terminal* This) {
    if (This->data->caret_style == CARETSTYLE_NOBLINK) {
        ...
        caretbm = CreateBitmap(This->data->font_width, This->data->font_height,
                               1, 1, bits);
        free(bits);
        CreateCaret(This->data->hwndMain, caretbm, This->data->font_height,
                    This->data->font_width);
    }
    ...
    if (This->data->caret_style == CARETSTYLE_LINE) {
        ...
        savepen = SelectObject(hdc, CreatePen(PS_SOLID, 0, RGB(255, 255, 255)));
...
#undef TN5250_FLIPEM
    ...
    if (This->data->selend.x < cr.left) {
        This->data->selend.x = cr.left;
    }
    if (This->data->selend.x > cr.right) {
        This->data->selend.x = cr.right;
    }
    if (This->data->selend.y < cr.top) {
        This->data->selend.y = cr.top;
    }
    if (This->data->selend.y > cr.bottom) {
        This->data->selend.y = cr.bottom;
    }

    if (This->data->selstr.x < cr.left) {
        This->data->selstr.x = cr.left;
    }
    if (This->data->selstr.x > cr.right) {
        This->data->selstr.x = cr.right;
    }
    if (This->data->selstr.y < cr.top) {
        This->data->selstr.y = cr.top;
    }
    if (This->data->selstr.y > cr.bottom) {
        This->data->selstr.y = cr.bottom;
    }
    ...
    This->data->selend.x =
        cx * This->data->font_width + (This->data->font_width - 1);
    cy = This->data->selend.y / This->data->font_height;
    This->data->selend.y =
        cy * This->data->font_height + (This->data->font_height - 1);
...
void win32_copy_text_selection(Tn5250Terminal* This, Tn5250Display* display) {
    ...
    while (ey > tn5250_display_height(display)) {
        ey--;
    }
    ...
    for (y = sy; y <= ey; y++) {
        ...
        for (x = sx; x <= ex; x++) {
            ...
            if (((c & 0xe0) == 0x20) || (c < 0x40 && c > 0x00) || (c == 0xff)) {
                ...
                c = tn5250_char_map_to_local(tn5250_display_char_map(display),
                                             c);
            }
            bp++;
            if (bp == bufsize) {
...
void win32_print_screen(Tn5250Terminal* This, Tn5250Display* display) {
    ...
    */
    ...
    GetDIBits(screenDC, bmap, 0, (UINT)bmp.bmHeight, bitmap, (BITMAPINFO*)&bi,
              DIB_RGB_COLORS);

    if (StretchDIBits(pd->hDC, 0, 0, w2, h2, 0, 0, w + 6, h + 6, (char*)bitmap,
...
void win32_move_caret_to(Tn5250Terminal* This, Tn5250Display* disp, short y,
    ...
    cx =
        MIN(x / This->data->font_width,
            ((tn5250_display_width(globDisplay) < 100) ? (80 - 1) : (132 - 1)));
    cy = MIN(y / This->data->font_height,
             ((tn5250_display_width(globDisplay) < 100) ? (RowsFor80Col - 1)
                                                        : (RowsFor132Col - 1)));
