...
int create_makefile(const char* infile, const char* outfile,
                    ...
                    const char* innosetupdir, const char* lib5250_objs,
                    const char* installdir, const char* package,
                    const char* version, const char* debug);
...
int main(unsigned argc, char** argv) {
    ...
    if (create_tn5250_config_h(TN5250_CONFIG_H_IN, "config.h", with_openssl,
                               binary_release, package, version) < 0) {
        exit(1);
    }
    ...
    if (create_makefile("Makefile.win.in", "Makefile", openssl_lib,
                        openssl_include, innosetupdir, lib5250_objs, installdir,
                        package, version, debug) < 0) {
        exit(1);
    }
    ...
    if (create_makefile("tn5250_innosetup.iss.in", "tn5250_innosetup.iss",
                        ...
                        lib5250_objs, installdir, package, version,
                        debug) < 0) {
        exit(1);
    }
...
int create_tn5250_config_h(const char* infile, const char* outfile,
    ...
    if (stat(infile, &st) == 0) { /* tn5250-config.h.in found */
        ...
        i = 0;
        while (mredefs[i].from != NULL) {
            s = manual_redef(mredefs[i].def, with_openssl, binary_release,
                             package, version);
            if (s[0] != '\0') {
                fprintf(out, "%s\n", s);
            }
